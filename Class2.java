        ...
        try {
            ...
            for (String asnValue : asns) {
                if (asnValue != null && !asnValue.isEmpty()) {
                    ...
                    // ✅ Get ASN ID for logging
                    WebElement asnIdElement = wait.until(ExpectedConditions.presenceOfElementLocated(
                            By.cssSelector("span[data-component-id='AsnId']")
                    ));
                    String asnId = asnIdElement.getText().trim();

                    // ✅ Check Received Quantity
                    WebElement receivedQtyElement = wait.until(ExpectedConditions.presenceOfElementLocated(
                            By.cssSelector("span[data-component-id='TotalReceivedQuantity']")
                    ));
                    String receivedQtyText = receivedQtyElement.getText().trim();
                    int receivedQty = 0;
                    try {
                        receivedQty = Integer.parseInt(receivedQtyText);
                    } catch (NumberFormatException e) {
                        System.out.println("Received quantity not a number, defaulting to 0.");
                    }

                    // ✅ Print validation result
                    if (receivedQty > 0) {
                        System.out.println("ASN: " + asnId + " → Received Quantity = " + receivedQty +
                                " → Proceeding to Related Links & LPN");
                        ...
                        System.out.println("ASN: " + asnId + " → Received Quantity = " + receivedQty +
                                " → Stopping at ASN screen (no further actions).");
                        break; // stop processing further ASNs
                    }
                }
            }
