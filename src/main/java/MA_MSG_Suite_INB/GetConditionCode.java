            ...
            while (rowIterator.hasNext()) {
                ...
                if (conditionCodes.contains("\"FI\"") || conditionCodes.contains("\"CR\"")) {
                    ...
                    try {
                        ...
                        Workbook restoreWb = WorkbookFactory.create(new FileInputStream(file)); // re-open original
                        // We already have `allData` but simplest: write rows back
                        Sheet restoreSheet = restoreWb.getSheet("ReportRCV");
                        for (int i = 0; i < allData.size(); i++) {
                            Row newRow = restoreSheet.getRow(i);
                            if (newRow == null) newRow = restoreSheet.createRow(i);
                            List<String> dataRow = allData.get(i);
                            for (int j = 0; j < dataRow.size(); j++) {
                                Cell c = newRow.getCell(j);
                                if (c == null) c = newRow.createCell(j);
                                c.setCellValue(dataRow.get(j));
                            }
                        }
