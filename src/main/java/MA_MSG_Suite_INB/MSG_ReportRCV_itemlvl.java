...
public class MSG_ReportRCV_itemlvl {
    ...
    private static void processReportFromExcel(String token, String filePath) {
        ...
        try {
            ...
            // ✅ Find Run_status and Testcase & LCID column dynamically
            int runStatusCol = -1;
            int lcidColumn = -1;
            int testcaseCol = -1;
            ...
            for (int i = 0; i < header.getLastCellNum(); i++) {
                ...
                if ("Run_status".equalsIgnoreCase(h)) runStatusCol = i;
                if ("Lcid".equalsIgnoreCase(h)) lcidColumn = i;
                if ("Testcase".equalsIgnoreCase(h)) testcaseCol = i;
            }

            if (runStatusCol == -1) throw new RuntimeException("No 'Run_status' column found in Excel!");

            if (lcidColumn == -1) {
                // If LCID column not found, create one
            ...
            while (rowIterator.hasNext()) {
                ...
                // ✅ Generate LPN and LCID
                String lcid = LPNgenerator.getNextLpn();

                // ✅ Write LCID back to Excel
            ...
            // ✅ Save LCID updates safely
            fis.close();
            try (FileOutputStream fos = new FileOutputStream(file)) {
                workbook.write(fos);
            }

            System.out.println("✅ LCIDs written successfully. Processed total rows: " + totalRows);
    ...
    private static boolean triggerReportAPI(String token, JSONObject body, String asnId) {
        try {
            org.apache.hc.client5.http.classic.HttpClient client = HttpClients.createDefault();
            org.apache.hc.client5.http.classic.methods.HttpPost post = new org.apache.hc.client5.http.classic.methods.HttpPost(REPORT_URL);
