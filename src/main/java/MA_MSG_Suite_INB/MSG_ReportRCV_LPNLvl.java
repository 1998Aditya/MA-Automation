...
public class MSG_ReportRCV_LPNLvl {
    ...
    private static void processReportFromExcel(String token, String filePath) {
        ...
        try {
            ...
            // ✅ Find LCID & Testcase column dynamically
            int lcidColumn = -1;
            int testcaseCol = -1;
            int runStatusCol = -1;
            ...
            if (runStatusCol == -1) {
                // create run status column if not exists
                runStatusCol = header.getLastCellNum();
                header.createCell(runStatusCol).setCellValue("Run_status");
            }
