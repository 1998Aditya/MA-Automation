...
public class MsgController {
    ...
    private void executeInboundSteps(String jobId, List<String> steps, String env) throws Exception {
            ...
            private LinkedHashSet<String> collectTestcasesFromWorkbook(String dataExcelPath) {
                ...
                try (FileInputStream fis = new FileInputStream(new File(dataExcelPath));
                    ...
                    for (int i = 0; i < wb.getNumberOfSheets(); i++) {
                        ...
                        orderedTcs.addAll(grouped.keySet()); // preserves insertion order
                    }
                } catch (Exception e) {
        ...
        try {
            // Fast-exit if stop requested already
            if (isStopRequested(jobId)) {
            ...
            if (testcases.isEmpty()) {
                ...
                for (String step : steps) {
                    ...
                    // Respect interrupt and stop flag
                    if (Thread.currentThread().isInterrupted() || isStopRequested(jobId)) {
                        System.out.println("Stopping inbound run for jobId=" + jobId + " before step " + step);
                        return;
                    }
                    ...
                    // Respect interrupt and stop flag after step
                    if (Thread.currentThread().isInterrupted() || isStopRequested(jobId)) {
                        System.out.println("Stopping inbound run for jobId=" + jobId + " after step " + step);
                        return;
                    }

                    // Use smaller sleeps to be more responsive to interrupts
                    try {
                        Thread.sleep(1000);
                    } catch (InterruptedException ie) {
                        Thread.currentThread().interrupt();
                ...
                for (String tc : testcases) {
                    ...
                    if (Thread.currentThread().isInterrupted() || isStopRequested(jobId)) {
                        System.out.println("Stop requested (jobId=" + jobId + ") — aborting before testcase: " + tc);
                        return;
                    }
                    ...
                    for (String step : steps) {
                        ...
                        if (Thread.currentThread().isInterrupted() || isStopRequested(jobId)) {
                            System.out.println("Stop requested (jobId=" + jobId + ") — halting execution before step: " + step + " in testcase: " + tc);
                        ...
                        if (Thread.currentThread().isInterrupted() || isStopRequested(jobId)) {
                            System.out.println("Stop requested (jobId=" + jobId + ") — stopping after step: " + step + " in testcase: " + tc);
                            System.clearProperty("testcase");
                            return;
                        }
                ...
                } // end per-testcase loop
            } // end if testcases present

        } finally {
            try {
                if (driver != null) {
                    // If a stop was requested or thread interrupted, quit driver immediately
                    if (isStopRequested(jobId) || Thread.currentThread().isInterrupted()) {
                        try {
                            driver.quit();
                        } catch (Exception e) {
                            System.out.println("Error quitting driver during stop: " + e.getMessage());
                        }
                        System.out.println("ChromeDriver closed due to stop request for jobId=" + jobId);
                    } else {
                        // preserve original behaviour when not stopping
                        try {
                            Thread.sleep(10000000);
                        } catch (InterruptedException ie) {
                            // If interrupted while sleeping, close driver and propagate
                            try { driver.quit(); } catch (Exception ignored) {}
                            Thread.currentThread().interrupt();
                            System.out.println("Interrupted during post-run sleep for jobId=" + jobId);
                            return;
                        }
                        try {
                            driver.quit();
        ...
        for (String tc : cases) {
            ...
            if (steps.contains("stepOB1A")) {
                ...
                if (e8.get(tc) != null) {
                    ...
                    MainA1_OrderReadyForPacking.main(tc,filePath, "OrderReadyForPacking");
                    Thread.sleep(800);
                }
            }
            if (steps.contains("stepOB2A")) {
                ...
                MainA2_oLPNPrepared.main(tc,filePath, "oLPNPrepared");
                Thread.sleep(800);
            }}
